...
class DAODetallefactutemp{
    ...
    public function InserDetallefact(){
        ...
        $ins .= ",'".$this->_num_fact."'";
        $query = 'INSERT INTO detallefactura_temp (cod_articulo,nom_articulo,valorve_articulo,canti_articulo,subtotal_articulo,
                  num_fact) VALUES ('.$ins.')';  
    ...
    public function Moddetallefat(){
        ...
        $query = 'UPDATE detallefactura_temp SET '.$upd.' WHERE cod_articulo='.$this->_cod_articulo.' AND num_fact='.$this->_num_fact;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    public function Infodetalletemp(){
        $query = 'SELECT * FROM detallefactura_temp WHERE cod_articulo='.$this->_cod_articulo.' AND num_fact='.$this->_num_fact;
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
    }

    public function InfoArticulosdetalletemp(){
        $query = 'SELECT * FROM detallefactura_temp WHERE cod_articulo='.$this->_cod_articulo;
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
    }
   
    public function InfoDetafact(){
        $query = 'SELECT * FROM detallefactura_temp WHERE num_fact='.$this->_num_fact;
        $this->_db->Query($query);
        $rows = array();
    ...
    public function BorrarDetallefact(){
        $query = "DELETE FROM detallefactura_temp WHERE cod_articulo=".$this->_cod_articulo." AND num_fact=".$this->_num_fact;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
