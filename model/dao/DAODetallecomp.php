...
class DAODetallecomp {
    ...
    public function BorrarDetallecomp(){
        $query = 'DELETE FROM detallecompra WHERE num_comp='.$this->_num_comp;
    ...
    public function InserDetallecomp(){
        ...
        $ins .= ",'".$this->_num_comp."'";
        $query = 'INSERT INTO detallecompra (cod_articulo,nom_articulo,valorve_articulo,canti_articulo,subtotal_articulo,
                  num_comp) VALUES ('.$ins.')';  
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
       ...
       public function ModiDetallecomp(){
        ...
        //$query = "UPDATE detallecompra SET ".$upd." WHERE num_comp='".$this->_num_comp."'";
        $query = "UPDATE detallecompra SET ".$upd." WHERE num_comp='".$this->_num_comp."'"."AND cod_articulo='".$this->_cod_articulo."'";
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    public function Buscarcodart(){
        $query = 'SELECT * FROM detallecompra WHERE cod_articulo='.$this->_cod_articulo;
        error_log($query);
         $this->_db->Query($query);
         $rows = array();
    ...
    public function AsignarNocompura(){
        $query = 'SELECT MAX(num_enca_comp) FROM  encabezadocompura';
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
        
    }

    public function ConsulUltcompuras(){
        $query = 'SELECT * FROM  encabezadocompura';
        $this->_db->Query($query);
        $rows = array();
        while ($row = $this->_db->FetchAssoc()) {
            $rows[] = $row;
    ...
    public function ModArtdetallefat(){
        $upd = "canti_articulo = '".$this->_canti_articulo."'";
        $upd .= ",valorve_articulo = '".$this->_valorve_articulo."'";
        $query = 'UPDATE detallecompra SET '.$upd.' WHERE cod_articulo='.$this->_cod_articulo;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
