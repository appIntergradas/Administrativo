...
class DAOUsuario_temp {
    ...
    public function VaciarUstemp(){
        $query= 'TRUNCATE usuarios_temp' ;
       ...
       public function InfoUsures(){
        $query = 'SELECT * FROM usuarios_temp WHERE usu_codigo='.$this->_usu_codigo;
            $this->_db->Query($query);
    ...
    public function ListarUsuarios($orden = 'ASC'){
        $query = 'SELECT * FROM usuarios_temp';
    ...
    public function ModificarUsuario(){
        ...
        $ins .= ",permiso_anular'".$this->_permiso_anular."'";
        $query = 'UPDATE usuarios_temp SET '.$upd.' WHERE usu_identificacion='.$this->_usu_identificacion;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    public function BorrarUsuario(){
        $query = 'DELETE FROM usuarios_temp WHERE usu_codigo='.$this->_usu_codigo;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    public function RegistrarUsuario(){
        ...
        $query = 'INSERT INTO usuarios_temp (usu_identificacion,usu_nombre,
    ...
    public function InfoUsuarios(){
        $query = "SELECT * FROM usuarios_temp WHERE usu_login='".$this->_usu_login."'";
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
    } 
