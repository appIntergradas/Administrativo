...
class  DAOFacturb {
    protected $_nfact;
    protected $_valor;
    protected $_cod_enca_fact;
    protected $_nit_enca_cl;
    protected $_nom_enca_cl;
    protected $_fecha_enca_fact;
    protected $_nom_enca_vendedor;
    protected $_num_enca_fact;
    ...
    public function DeleFacturb(){
        $query = 'DELETE FROM facturarb WHERE nfact='.$this->_nfact;
    ...
    public function ListarFactureciboTodos($orden = 'ASC'){
        $query = 'SELECT * '
                . 'FROM facturarb ';
    ...
    public function VaciarTablafactrecibo(){
        $query= 'TRUNCATE facturarb' ;
        $this->_db->Query($query);
            if($this->_db->FilasAfectadas()>0){
                return true;
            }else{
    ...
    public function InsertFacturecibo(){
        $ins = "'".$this->_nfact."'";
        $ins .= ",'".$this->_valor."'";
        $query = 'INSERT INTO facturarb (nfact,valor) VALUES ('.$ins.')';  
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    public function InfoEncafact(){
        $query = 'SELECT * FROM encabezadofactura WHERE num_enca_fact='.$this->_num_enca_fact;
        $this->_db->Query($query);
    ...
    public function InserEncafact(){
        $ins = "'".$this->_nit_enca_cl."'";
        $ins .= ",'".$this->_nom_enca_cl."'";
        $ins .= ",'".$this->_fecha_enca_fact."'";
        $ins .= ",'".$this->_nom_enca_vendedor."'";
        $ins .= ",'".$this->_num_enca_fact."'";
        $ins .= ",'".$this->_subtota_enca."'";
        $ins .= ",'".$this->_total_enca."'";
        $ins .= ",'".$this->_forma_pago."'";
        $query = 'INSERT INTO encabezadofactura (nit_enca_cl,nom_enca_cl,fecha_enca_fact,nom_enca_vendedor,num_enca_fact,
    ...
    public function ModiEncafactanul(){
    ...
    public function ConsulUltfacturasvende(){
    ...
    public function ConsulUltfacturas(){
        $query = 'SELECT * FROM  encabezadofactura  ORDER BY cod_enca_fact DESC LIMIT 5';
        $this->_db->Query($query);
        $rows = array();
        while ($row = $this->_db->FetchAssoc()) {
            $rows[] = $row;
    ...
    function get_num_enca_fact() {
        return $this->_num_enca_fact;
    }
    ...
    function get_fecha_enca_fact() {
        return $this->_fecha_enca_fact;
    }
    ...
    function set_nfact($_nfact) {
        $this->_nfact = $_nfact;
    }
    function set_valor($_valor) {
        $this->_valor = $_valor;
    }
    function set_num_enca_fact($_num_enca_fact) {
        $this->_num_enca_fact = $_num_enca_fact;
    }
    ...
    function set_fecha_enca_fact($_fecha_enca_fact) {
        $this->_fecha_enca_fact = $_fecha_enca_fact;
    }
