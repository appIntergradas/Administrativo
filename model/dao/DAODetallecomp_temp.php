 ...
 * @author aquirozr
 */
class DAODetallecomptemp{
    protected $_cod_detalle_comp;
    ...
    public function InserDetallecomp(){
        ...
        $ins .= ",'".$this->_num_comp."'";
        $query = 'INSERT INTO detallecompra_temp (cod_articulo,nom_articulo,valorve_articulo,canti_articulo,subtotal_articulo,
                  num_comp) VALUES ('.$ins.')';  
    ...
    public function Moddetallecom(){
        ...
        $query = 'UPDATE detallecompra_temp SET '.$upd.' WHERE cod_articulo='.$this->_cod_articulo.' AND num_comp='.$this->_num_comp;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    public function Infodetalletemp(){
        $query = 'SELECT * FROM detallecompra_temp WHERE cod_articulo='.$this->_cod_articulo.' AND num_comp='.$this->_num_comp;
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
    }

    public function InfoArticulosdetalletemp(){
        $query = 'SELECT * FROM detallecompra_temp WHERE cod_articulo='.$this->_cod_articulo;
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
    }
   
    public function InfoDetacomp(){
        $query = 'SELECT * FROM detallecompra_temp WHERE num_comp='.$this->_num_comp;
        $this->_db->Query($query);
        $rows = array();
    ...
    public function ModArtdetallecom(){
        $upd = "canti_articulo = '".$this->_canti_articulo."'";
        $upd .= ",valorve_articulo = '".$this->_valorve_articulo."'";
        $upd .= ",subtotal_articulo = '".$this->_subtotal_articulo."'";
        $query = 'UPDATE detallecompra_temp SET '.$upd.' WHERE cod_articulo='.$this->_cod_articulo.' AND num_comp='.$this->_num_comp;
    ...
    public function BorrarDetallecomp(){
        $query = "DELETE FROM detallecompra_temp WHERE cod_articulo=".$this->_cod_articulo." AND num_comp=".$this->_num_comp;
        $this->_db->Query($query);
        if($this->_db->FilasAfectadas()>0){
            return true;
        }else{
    ...
    function set_num_comp($_num_comp) {
        $this->_num_comp = $_num_comp;
    }
