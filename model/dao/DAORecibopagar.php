...
class DAORecibopagar {
    ...
    public function AsignarNorecibo(){
        $query = 'SELECT MAX(num_factrb) FROM  reciboapagar';
    ...
    public function ModiRecipago(){
        ...
        $query = "UPDATE reciboapagar SET ".$upd." WHERE num_factrb='".$this->_num_factrb."'";
        $this->_db->Query($query);
    ...
    public function InserRecipago(){
        ...
        $query = 'INSERT INTO reciboapagar (num_factrb,concepto_factrb,valor_factrb,averias_factrb,descu_factrb,
    ...
    public function ListarRecipago(){
        $query = "SELECT * FROM reciboapagar";
        $this->_db->Query($query);
        $rows = array();
    ...
    public function VaciarRecipago(){
       ...
       public function InfoRecipago(){
        $query = 'SELECT * FROM reciboapagar WHERE num_factrb='.$this->_num_factrb;
        $this->_db->Query($query);
        return $this->_db->RowFetchArray();
    }

       public function DeleRecipago(){
        $query = 'DELETE FROM reciboapagar WHERE num_factrb='.$this->_num_factrb;
        $this->_db->Query($query);
            if($this->_db->FilasAfectadas()>0){
                return true;
            }else{
